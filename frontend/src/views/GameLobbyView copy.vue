...
// Función auxiliar para actualizar stores de Pinia con datos de la partida
const updateStoresWithGameData = (gameData: typeof game.value) => {
  if (!gameData) return
  
  // Actualizar gameStore
  console.log('Updating gameStore with game data:', gameData)
...
const joinGame = async () => {
  ...
  if (game.value) {
    // Convertir players a formato compatible con initializePlayersStatus
    ...
    // Actualizar stores de Pinia después de unirse
...
onMounted(async () => {
  // Cargar datos de la partida mediante 
  await loadGame()
  
  // Si se cargó correctamente, inicializar estado de jugadores y actualizar stores
  if (game.value) {
    // Convertir players a formato compatible con initializePlayersStatus
    const playersArray = game.value.players.map(player => ({ 
      id: player.player_id, 
  ...
  // No tengo claro si es necesaria y si el backend acepta mensajes de websocket.
  // setTimeout(() => {
  //   notifyUserJoinedLobby()
  // }, 1000)
})
